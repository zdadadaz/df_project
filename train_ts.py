...
def run_epoch(model, dataloader, phase, optim, device, save_all=False, batch_size=8):
    ...
    counter = 0
    train_corrects = 0.0
    h_t, c_t = model.module.zero_state(batch_size)
    h_t = h_t.float().to(device)
...
# Data preparation
    train_dataset = deepfake_ts(split="train", **kwargs, pad=12)
    val_dataset = deepfake_ts(split="test", **kwargs)
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        ...
        for epoch in range(epoch_resume, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                loss, accuracy, yhat, y = run_epoch(model, dataloaders[phase], phase, optim, device, batch_size = batch_size)
