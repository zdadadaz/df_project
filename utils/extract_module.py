...
class Extract_image():
    def __init__(self, jsons, dirs, dir_out):
        self.detector = MTCNN(margin=20, keep_all=True, post_process=False, device='cuda:0')
        self.margin = 0
    ...
    def read_existed_files(self, out_dirname):
        ...
        for fake_real in os.path.listdir(out_dirname):
            for f in os.path.listdir(os.path.join(out_dirname,fake_real)):
                if  (f[-4:] == '.jpg'):        
    ...
    def extract(self):
        for i in range(1,self.total_files):
            ...
            if vid.split('.')[0] in self.existed_filenames:
                ...
                continue
            if os.path.isfile(os.path.join(self.dir_lists, vid)) and ((vid[-4:] == '.mp4') or (vid[-4:] == '.avi') or (vid[-4:] == '.mov')):
                out_filename = os.path.join(self.dir_out,self.train_list.loc[i]['label'])
        ...
        for cr in range(2):
            ...
            if len(faces) == len(images) or cr == 1:
                for i in range(len(faces)):
                    ...
                    cv2.imwrite(os.path.join(out_filename,output_name), cv2.cvtColor(faces[i], cv2.COLOR_RGB2BGR))
                    # imageio.imwrite(os.path.join(out_filename,output_name),faces[i],'jpg')
                break
